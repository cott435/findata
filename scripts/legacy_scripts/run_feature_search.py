...
from findata.analysis.search import DEFAULT_VARIANTS, default_variants
...
def parse_args():
    ...
    p.add_argument("--include-modes", action="store_true",
                   help="add the ticker-space mode-decomposition variants "
                        "(modes / cs_whiten; needs sector labels)")
    return p.parse_args()
...
def main():
    ...
    data, ticker_info = get_all_data(tickers, splits.data_start, splits.data_end)
