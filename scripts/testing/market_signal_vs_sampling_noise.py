...
def fit_bulk_sigma2(eigs: np.ndarray, q: float) -> float:
    """After removing eigenvalues that leak past the naive (sigma2=1) MP edge,
    the remaining trace is the noise budget. sigma2_bulk = mean(bulk eigs).
    This is the *right* way to get sigma2 -- NOT a KDE/curve-fit search."""
    _, hi = mp_bounds(q, 1.0)
    bulk = eigs[eigs <= hi]
    return float(bulk.mean()) if bulk.size else 1.0


# ---------------------------------------------------------------------------
# 6. Deliverables
# ---------------------------------------------------------------------------
def run_analysis(price_df: pd.DataFrame, price_col: str | None = None,
                 out_prefix: str = "rmt", seed: int = 0):
    folder = 'market_signal_vs_sampling_noise'
    returns = panel_to_returns(price_df, price_col)
    T, N = returns.shape
    q = N / T

    X = standardize(returns)
    C = correlation_matrix(X)
    eigs = eigvals_desc(C)

    null_ret = shuffle_null(returns, seed=seed)
    Cn = correlation_matrix(standardize(null_ret))
    eigs_n = eigvals_desc(Cn)

    lo, hi = mp_bounds(q, 1.0)
    sigma2 = fit_bulk_sigma2(eigs, q)
    lo_s, hi_s = mp_bounds(q, sigma2)
    n_signal = int((eigs > hi_s).sum())
    ...
    ax.legend()
    fig1.tight_layout()
    fig1.savefig(EXPERIMENT_DIR / folder / f"{out_prefix}_1_Cij_distribution.png", dpi=130)
    ...
    fig2.tight_layout()
    fig2.savefig(EXPERIMENT_DIR / folder / f"{out_prefix}_2_eigenvalue_density.png", dpi=130)
...
if __name__ == "__main__":
    ...
    dates = DataSplits()
    data = get_price_data(tickers, dates.data_start, dates.data_end)
    prices = data[["close"]]
    print(prices.head(), "\n")
    run_analysis(prices, price_col="close", out_prefix="rmt")
    print("\nsaved: rmt_1_Cij_distribution.png, rmt_2_eigenvalue_density.png")
